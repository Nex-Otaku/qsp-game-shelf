...
class AssignmentController extends Controller
{
    ...
    {
        $class = $this->module->userClassName;
        $binaryId = uuid::uuid2bin($id);
        $model = $class::findIdentity($binaryId);
        if($model === null){
        ...
        $assignment = [];
        foreach ($manager->getAssignments($binaryId) as $item) {
            $assignment[] = $items[$item->roleName];
